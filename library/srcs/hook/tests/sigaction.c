...
int EXPORT sigaction(int signum, const struct sigaction *act, struct sigaction *oldact)
{
    typeof(&sigaction) original_function = get_original_function();
    if (!is_hooks_enabled())
        return original_function(signum, act, oldact);
